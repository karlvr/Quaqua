...
public class Quaqua14PopupMenuUI extends BasicPopupMenuUI implements QuaquaMenuPainterClient {
    ...
    public Popup getPopup(JPopupMenu popup, int x, int y) {
        Popup popi;
        Float alpha = (Float) popup.getClientProperty("Quaqua.PopupMenu.alpha");
        if (alpha == null) {
            alpha = new Float(0.75f);
        }
        if (alpha.floatValue() == 1f || !QuaquaManager.getBoolean("PopupMenu.enableHeavyWeightPopup")) {
            popi = new QuaquaPopupFactory().getPopup(popup.getInvoker(), popup, x, y);
        } else {
            popi = super.getPopup(popup, x, y);
        }
        return popi;
    }
        ...
        void grabWindow(MenuElement[] newPath) {
            ...
            java.security.AccessController.doPrivileged(
                    new java.security.PrivilegedAction() {
                        ...
                        public Object run() {
                            Toolkit.getDefaultToolkit().addAWTEventListener(MouseGrabber.this,
            ...
            switch (ev.getID()) {
                case MouseEvent.MOUSE_PRESSED:
                    ...
                    if (!(src instanceof JComponent) ||
                            !(((JComponent) src).getClientProperty("doNotCancelPopup") == QuaquaComboBoxUI.HIDE_POPUP_KEY)) {
